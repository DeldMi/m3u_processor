...
14. Sempre que faser uma atualização no projeto fucionado fazer brecap com a veção e usar a rai (main) como test depos de valida e fazer a auditoria vamos muda isso para um de teste e na raiz fica o projeto com a vesao atual e fucional.
15. A base deste porjeto e analizar, testar, prosesalo e gera links tem que ter uma administrção compreta (Analise teste edição criação etc).
