...
export function Notifications() {
    ...
    useEffect(() => {
        items.slice(0, 8).forEach((item) => {
            ...
            if (!hidden[key] && !timers.current[key]) schedule(key);
        });
    }, [items, hidden]);
    const pause = (key: string) => { window.clearTimeout(timers.current[key]); setPaused((old) => ({ ...old, [key]: true })); };
    const resume = (key: string) => { setPaused((old) => ({ ...old, [key]: false })); schedule(key); };
