...
# ConfigManager garante um SECRET_KEY persistente no .env; não existe fallback público.
app.secret_key = ConfigManager(BASE_DIR).get_all()["SECRET_KEY"]
...
def react_assets(filename):
    """Serve os assets React nos dois prefixes históricos."""
...
def view_settings():
    ...
    if react_app:
        return react_app
    return render_template("settings.html", config=manager.config_mgr.get_public())
