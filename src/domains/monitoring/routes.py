...
def register_monitoring_routes(app, manager, process_state, process_logs, notifications, monitor: ResourceMonitor, add_notification=None):
    ...
    def resource_gc():
        ...
        notice = add_notification or (lambda level, title, message, source="Execução", path="", details="": notifications.append({"id": f"{datetime.now().timestamp()}-memory", "level": level, "title": title, "message": message, "timestamp": datetime.now().isoformat(timespec="seconds"), "source": source, "path": path, "details": details}))
