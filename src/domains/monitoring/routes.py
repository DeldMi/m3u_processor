...
def register_monitoring_routes(app, manager, process_state, process_logs, notifications, monitor: ResourceMonitor, add_notification=None):
    ...
    def get_themes():
        try:
            value = json.loads(str(manager.config_mgr.get_all().get("CUSTOM_THEMES", "[]")))
    ...
    def create_channel():
        ...
        if channel_id is None:
            return jsonify({"error": "Canal inválido ou URL já cadastrada."}), 400
        ...
        if add_notification:
            add_notification("success", "Canal criado", message, source="Canais", path="/channels")
    ...
    def delete_channel(channel_id):
        if not manager.db.delete_channel(channel_id):
        ...
        if add_notification:
            add_notification("warning", "Canal removido", message, source="Canais", path="/channels")
